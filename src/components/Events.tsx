...
export default function Novedades() {
  ...
  const lightboxRef = useFocusTrap(!!lightboxData);
  ...
  return (
    <section id="eventos" aria-label="Eventos" className="relative py-16 px-6">
      ...
      <div className="relative max-w-7xl mx-auto z-10">
            ...
            return (
              <button
                ...
                className={`relative shrink-0 px-4 py-2 rounded-full font-body text-sm font-medium transition-all duration-300 cursor-pointer ${
                  isSelected
                    ? "bg-tlahu-clay text-white"
                    : "bg-ash-gray text-[#374151] border border-[#fff5dd] hover:bg-tlahu-gold/20"
                } ${month.icon ? "pr-7" : ""}`}
              >
                {month.icon && (
                  <img
        ...
        {/* Events */}
        {loading || error ? (
          <div className="min-h-125 animate-pulse" aria-busy="true" aria-live="polite">
            {error && (
          ...
          <div className="min-h-125">
            {events.length === 0 ? (
              <div className="text-center py-16">
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-ash-gray mb-4">
                  <IconCalendar className="text-slate-gray" />
                </div>
                <p className="font-body text-lg text-slate-gray mb-2">
                  No hay eventos programados
                </p>
                <p className="font-body text-sm text-slate-gray/70">
              ...
              <>
                ...
                <div className="hidden md:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {formattedEvents.map((event) => (
                    <EventCard key={event.id} event={event} onImageClick={openLightbox} />
                  ))}
                </div>
              </>
            )}
